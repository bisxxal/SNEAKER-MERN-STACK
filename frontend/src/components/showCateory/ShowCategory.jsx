...
function ShowCategory({categorie , setCatrgorie}) {
  return (
    <div>
         <div className='w-full flex flex-col lg:py-10 py-6  gap-3 px-4 lg:px-0  border-b-[1px] border-[#ffffff76] '>
        ...
        <div className='item w-full flex items-center justify-between gap-4 lg:gap-[unset] mt-5 overflow-x-auto  '>
            ...
            className=' flex-shrink-0  flex flex-col items-center gap-4 h-[200px] w-[180px]  bg-zinc-300 rounded-lg overflow-hidden ' key={index}>
                ...
                <p className={`text-[13px] lg:text-[15px]  ${categorie === item.name ?"text-[#FF1E1E]":''} font-bold text-[#353535] cursor-pointer `}>{item.name}</p>
              </div> )}
        </div>
    </div>
    </div>
